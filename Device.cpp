...
VkDevice Device::createDevice(const std::vector<const char*>& deviceExtensions, const std::vector<const char*>& layers)
{
...
Device::Device(VkInstance instance, VkSurfaceKHR surface, const std::vector<const char*>& deviceExtensions, const std::vector<const char*>& layers)
	: queueFamilyIndices()
	, surface(surface)
	, physicalDevice(pickPhysicalDevice(instance))
	, device(createDevice(deviceExtensions, layers))
	, graphicsQueue()
	, presentQueue()
	, commandPool()
	, descriptorPool()
...
void swap(Device& a, Device& b)
{
    ...
    std::swap(a.queueFamilyIndices, b.queueFamilyIndices);
    std::swap(a.surface, b.surface);
    std::swap(a.physicalDevice, b.physicalDevice);
	std::swap(a.device, b.device);
	std::swap(a.graphicsQueue, b.graphicsQueue);
	std::swap(a.presentQueue, b.presentQueue);
    std::swap(a.commandPool, b.commandPool);
    std::swap(a.descriptorPool, b.descriptorPool);
}
